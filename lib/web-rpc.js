  ...
  return {
    async fetch(request) {
      ...
      try {
        ...
        log.error?.('dsh-pocket-relay: rpc %s failed | RPC 失败: %s', endpoint, err?.message ?? err);
...
export function installPocketRpc(ctx, { service, log = console, desktop = false, runUpdate = null, restart = null, restartNotice = null, getToken = null, getLanToken = null, refreshLanToken = null, getLanAuthEnabled = null, setLanAuthEnabled = null, getLanEnabled = null, setLanEnabled = null, getLanIpOverride = null, setLanIpOverride = null, getPinCustom = null, setCustomPin = null, getTunnelConfig = null, setTunnelConfig = null, resetPocket = null, getRelayConfig = null, setRelayConfig = null }) {
  const pocketRpcHandler = async (endpoint, payload = {}, signal) => {
    ...
    const statusPayload = async () => {
      ...
      return ok({
        ...
        // 中继脱敏视图：token 只写不读，只回 tokenSet 布尔（与隧道 Token 同款语义）
        relay: relayView(),
        // 设备计数（列表本身走 device.list，避免每次 3 秒轮询都拖着全量设备数据）
        deviceStatus: deviceStore()?.status() ?? { devices: 0, pending: 0, pairings: 0, sessions: 0 },
      });
    };

    /** 中继配置的脱敏视图（永不回显 token 原文）。 */
    const relayView = () => {
      ...
      return {
        ...
        // 本机在 relay 上的名字（多机共存用）。空 = 按主机名派生，界面上
        // 显示运行态里的实际值即可（relayState.agentId），所以这里回空字符串。
        agentId: cfg.agentId ?? '',
      };
    };

    /** 设备存储句柄（service 暴露；测试可注入 internals.deviceAuth）。 */
    const deviceStore = () => service?.deviceAuth ?? null;

    /**
     * 设备列表视图。**永不包含 tokenHash / password**——device-auth.mjs 的
     * 公开行本身就不带这些字段，这里只是原样搬出来。
     */
    const deviceListPayload = () => {
      const store = deviceStore();
      if (!store) return { devices: [], pending: [], status: { devices: 0, pending: 0, pairings: 0, sessions: 0 } };
      return {
        devices: store.list().filter((d) => d.approved),
        pending: store.pending(),
        status: store.status(),
      };
    };

    /** 配对二维码要指向的地址：中继的对外访问地址。 */
    const pairUrlFor = (code) => {
      const base = String((getRelayConfig?.() ?? {}).publicUrl ?? '').replace(/\/+$/, '');
      return base ? `${base}/pocket-pair?code=${encodeURIComponent(code)}` : null;
    };

    /**
     * 让运行态跟上配置态：enabled 且配置完整 → 重连；否则 → 停。
     *
     * 一律「先停再起」，不比较配置差异：中继 agent 是纯出站长连接，
     * 重建成本极低（毫秒级），而漏判某个字段（比如 tls 从 false 翻成 true）
     * 会留下一条用旧参数连着的连接，用户看到的是「改了没生效」。
     *
     * 返回 { ok, message }：失败**不抛**，由调用方转成 RPC 错误，
     * 以免中继连不上把整个设置页打挂。
     */
    const applyRelayState = async () => {
      const cfg = getRelayConfig?.() ?? {};
      if (!cfg.enabled) {
        service.stopRelay?.();
        return { ok: true };
      ...
      if (!cfg.host || !cfg.port || !cfg.token) {
        service.stopRelay?.();
        return { ok: false, message: '请先填写中继地址、端口和 Token | fill in the relay address, port and token first' };
      }
      try {
        service.stopRelay?.();       // 先停：保证用新参数重连
    ...
    try {
      ...
      if (endpoint === POCKET_ENDPOINTS.relaySetConfig) {
        // 中继配置：{ host, port, token, publicUrl, agentId, tls, allowInsecure, enabled, consent }
        // token 留空 = 保持已存值（界面不回显）。返回完整 status 供前端直接替换。
        if (!setRelayConfig) return fail('bad-request', '中继配置不可用 | relay config unavailable');
        try {
          setRelayConfig({
            host: payload?.host,
            port: payload?.port,
            ...
            tls: payload?.tls,
            allowInsecure: payload?.allowInsecure,
        ...
        // 配置变了 → 先用新配置重连一次，保证「改完就是生效的」，
        // 而不是等下次重启 dsh web 才生效（常开这个诉求下，用户不该关心重启）。
        const applied = await applyRelayState();
        if (!applied.ok) return fail('bad-request', applied.message);
        return await statusPayload();
      }
      if (endpoint === POCKET_ENDPOINTS.relaySetEnabled) {
        // 常开开关（「暂停远程」/「恢复」）。暂停只断中继，不影响局域网与公网。
        if (!setRelayConfig) return fail('bad-request', '中继不可用 | relay unavailable');
        try {
          setRelayConfig({ enabled: payload?.on === true });
        } catch (err) {
          return fail('bad-request', err?.message ?? String(err));
        }
        const applied = await applyRelayState();
      ...
      if (endpoint === POCKET_ENDPOINTS.deviceList) {
        return ok(deviceListPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.devicePairingCreate) {
        const store = deviceStore();
        if (!store) return fail('bad-request', '设备认证不可用 | device auth unavailable');
        // 二维码必须指向用户手机能打开的地址；没配对外地址就生成的码是废的，
        // 与其让用户扫了打不开，不如在这里说清楚。
        if (!pairUrlFor('x')) {
          return fail('bad-request', '请先在中继设置里填写「对外访问地址」，否则配对二维码无法使用 | set the public URL first');
        }
        const out = store.createPairing({ name: payload?.name });
        if (out.error) return fail('bad-request', out.error);
        const pairUrl = pairUrlFor(out.code);
        // 二维码在这里生成：手机要扫的就是它，让前端再走一次 RPC 拿图没必要
        const pairQr = await qrDataUrl(pairUrl).catch(() => null);
        return ok({ ...out, pairUrl, pairQr, ...deviceListPayload() });
      }
      if (endpoint === POCKET_ENDPOINTS.devicePairingCancel) {
        const store = deviceStore();
        if (!store) return fail('bad-request', '设备认证不可用 | device auth unavailable');
        store.cancelPairing?.();
        return ok(deviceListPayload());
      ...
      if (endpoint === POCKET_ENDPOINTS.deviceApprove
        || endpoint === POCKET_ENDPOINTS.deviceReject
        || endpoint === POCKET_ENDPOINTS.deviceRevoke) {
        const store = deviceStore();
        if (!store) return fail('bad-request', '设备认证不可用 | device auth unavailable');
        const id = String(payload?.id ?? '').trim();
        if (!id) return fail('bad-request', '缺少设备 id | missing device id');
        const done = endpoint === POCKET_ENDPOINTS.deviceApprove ? store.approve(id)
          : endpoint === POCKET_ENDPOINTS.deviceReject ? store.reject(id)
            : store.revoke(id);
        if (!done) return fail('bad-request', '设备不存在或状态不匹配 | device not found or wrong state');
        return ok(deviceListPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.pocketReset) {
        // 恢复出厂设置：必须显式确认（payload.confirm === true），先停隧道与中继再清空设置与密码，
        ...
        try {
          service.stopTunnel();
          service.stopRelay?.();
          // 设备凭据也是本机状态的一部分：恢复出厂要一并撤销，否则「重置」之后
          // 之前配对的手机还能照常进来，用户会以为重置没生效。
          service.resetDevices?.();
          resetPocket();
          return await statusPayload();
        } catch (err) {
          return fail('bad-request', err?.message ?? String(err));
  ...
  if (!ctx?.connection?.rpc?.handle) {
    log.warn?.('dsh-pocket-relay: DSH Host Connection RPC unavailable — settings tab disabled | 无 Connection RPC，设置页不可用');
    return () => {};
  }
