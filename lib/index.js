// dsh-pocket-relay 插件入口（单包单插件：手机扫码访问 DSH + 自建 relay 远程，全在这一个包里）
//
// 设置一级入口「手机访问」：
//   - 局域网二维码：自动显示（代理随插件启动）
...
import { lanEnabled, setLanEnabled, lanAuthEnabled, setLanAuthEnabled, lanIpOverride, setLanIpOverride, pinCustom, setPinCustom, tunnelMode, setTunnelMode, tunnelToken, setTunnelToken, tunnelHostname, setTunnelHostname, resetSettings, proxyPort, cloudflaredPath, relayConfig, setRelayConfig } from './settings.mjs';
...
/** 执行更新：dsh plugin --profile <p> update dsh-pocket-relay --latest -w（超时保护）。 */
function performUpdate(profile, { timeoutMs = 180_000 } = {}) {
  return new Promise((resolve) => {
    const child = spawn('dsh', ['plugin', '--profile', profile, 'update', 'dsh-pocket-relay', '--latest', '-w'], {
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  if (!dshPort) {
    logger.error('dsh-pocket-relay: webServer port unavailable — cannot start proxy | 拿不到 dsh web 端口，无法启动代理');
    return () => {};
  }
  ...
  if (cfPath) {
    ...
    logger.info(`dsh-pocket-relay: using custom cloudflared at ${cfPath} (skipping auto-download) | 使用自定义 cloudflared 路径（跳过自动下载）`);
  }
  ...
  if (isDesktop) {
    logger.info('dsh-pocket-relay: DSH Desktop detected — update/restart disabled here | 检测到桌面端环境，更新/重启已关闭');
  }
  ...
  if (desktopAdvanced) {
    logger.warn('dsh-pocket-relay: DSH Desktop advanced mode — phone access unsupported, injecting notice | 桌面端 advanced 模式：手机访问暂不支持，已注入提示');
  }
  ...
  const service = internals.service ?? createPocketService({
    ...
    // 中继配置：service 启动时常驻连接（「无人监听」靠它）
    getRelayConfig: () => relayConfig(),
    // 隧道就绪回调：快速模式每次开启轮换 8 位密码（旧链接作废）；命名模式地址固定，
    // 不自动轮换（重启后密码不变，用户可用「自定义密码」主动更换）。
    onTunnelReady: (mode) => {
      if (mode === 'named') {
        logger.info('dsh-pocket-relay: named tunnel ready — PIN not rotated | 命名隧道就绪（公网密码不自动轮换）');
        return getAccessToken();
      }
      const fresh = rotateAccessToken();
      logger.info('dsh-pocket-relay: public access PIN refreshed | 公网访问密码已更新（自定义密码不受影响）');
      return fresh;
    },
  });
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    // 中继（自建服务器）：这里给的是**完整**配置（含 token），
    // 因为 relayView() 要据它算 tokenSet、applyRelayState() 要据它判配置是否完整；
    // 对外回显一律走 web-rpc 的 relayView() 脱敏，token 永不进响应体。
    getRelayConfig: () => relayConfig(),
    setRelayConfig: (patch) => setRelayConfig(patch),
  ...
  void service.startProxy().then((proxy) => {
    logger.info('dsh-pocket-relay: proxy ready on :%d | 局域网代理已就绪', proxy.port);
    // 自动恢复上次开启的公网隧道（DSH 重启后 cloudflared 子进程被杀，issue #11）
    void service.restoreTunnelIfNeeded?.().catch(() => {});
    // 中继：配置好就直接连，**不等用户点任何按钮**——这就是「无人监听」。
    // 放在 startProxy 之后：relay 的流量要打到代理上，代理必须先就绪。
    // 失败只记日志、不抛出：中继连不上不该拖垮整个插件（局域网照常可用）。
    const rc = relayConfig();
    if (rc.enabled && rc.host && rc.port && rc.token) {
      void service.startRelay().catch((err) => {
        logger.warn('dsh-pocket-relay: relay start failed | 中继启动失败: %s', err?.message ?? err);
      });
    }
  }).catch((err) => {
    logger.error('dsh-pocket-relay: proxy start failed | 代理启动失败: %s', err?.message ?? err);
  });

  ctx.effect(() => async () => {
    for (const d of disposers.reverse()) { try { d(); } catch { /* 忽略 */ } }
    await service.dispose();
  }, 'dsh-pocket-relay: stop proxy and tunnel');
}
