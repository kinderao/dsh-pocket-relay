...
/**
 * 非本机来源（局域网 / 公网 / relay）**唯一**允许调用的 pocket 端点。
 *
 * `/dsh-pocket` 是设置页通道，会返回公网/局域网 PIN（pocket.status）、能改开关、
 * 能管设备。它本来是给「电脑上的设置页」用的，而电脑走 127.0.0.1 —— 也就是
 * loopback。手机虽然有同一个界面，但没有任何理由调用这条通道。
 *
 * 移动端确实需要它，但只需要一个端点：复制文件内容（issue #17，走 RPC 让主机读文件）。
 * 所以这里用**白名单**而不是黑名单：以后往 /dsh-pocket 加新端点时，默认是
 * 「远程不可达」，不会因为忘记登记而悄悄多开一个口子。
 *
 * 背景：DSH 宿主的信任栅栏只看 Host/Origin，而本代理会把远程请求的 Host 改写成
 * loopback 权威（那是它存在的意义），所以栅栏在远程场景下必然放行 —— 这条
 * 边界只能由代理自己守。
 */
export const REMOTE_ALLOWED_POCKET_ENDPOINTS = new Set(['pocket.fileRead']);

/**
 * 该请求是否命中设置页通道（`/dsh-pocket/<endpoint>`）且端点不在白名单内。
 * 路径形如 `/dsh-pocket/pocket.status`（endpoint 就在 path 里，无需解析 body）。
 *
 * @param {string} reqUrl
 * @param {Set<string>} allowed
 */
export function isPocketChannelPath(reqUrl, allowed) {
  let pathname;
  try {
    pathname = new URL(reqUrl ?? '/', 'http://dsh.invalid').pathname;
  } catch {
    pathname = String(reqUrl ?? '/').split('?')[0];
  }
  if (pathname !== '/dsh-pocket' && !pathname.startsWith('/dsh-pocket/')) return false;
  const endpoint = pathname.slice('/dsh-pocket'.length).replace(/^\/+/, '');
  if (endpoint === '') return true;   // 裸通道路径也一律拒（没有合法用途）
 ...
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true, launchToken = () => '', handshakeLimit, deviceAuth = null } = {}) {
  ...
  const server = createServer((req, res) => {
    // 处理函数是 async（设备认证要读 body / 算 scrypt），而 createServer 不消费
    // 返回的 promise —— 必须自己兜住 rejection，否则一个异常就是一个
    // unhandledRejection，在 Node 默认策略下会直接干掉 dsh web 进程。
    void handleRequest(req, res).catch((err) => {
      log?.(`dsh-pocket: request failed | 请求处理失败: ${err?.message ?? err}`);
      try {
        if (!res.headersSent) {
  ...
  async function handleRequest(req, res) {
    ...
    // 注入内容可以按 host 定制（设备通道要额外注入「真实用户活动」上报脚本）
    const inject = typeof injectHtml === 'function' ? injectHtml(host) : injectHtml;
    ...
    // 设备认证通道（自建 relay）：整条通道先过设备闸门。
    //
    // 闸门必须覆盖该 host 上的**所有**请求，而不只是首页：如果只拦 `/`，
    // 拿到真实域名的攻击者绕开首页直接打 `/api/...` 或 WebSocket 就进去了。
    // 认证通过则整段跳过下面的共享 PIN 校验（设备认证是它的替代，不是叠加）。
    let deviceChannel = false;
    if (deviceAuth && deviceAuth.handles(host)) {
      deviceChannel = true;
      const verdict = await deviceAuth.gate(req, res);
      if (verdict !== 'continue') return;
    }

    // 设置页通道（/dsh-pocket）只对**本机**开放，白名单端点除外。
    // 放在设备闸门之后：设备通道未认证时上面已经拦掉了，走到这里的是
    // 「已通过设备认证的手机」或「局域网/公网的手机」——它们同样不该拿到
    // pocket.status（会泄露公网 PIN）或设备管理端点。
    if (classifyHost(host) !== 'loopback' && isPocketChannelPath(req.url, REMOTE_ALLOWED_POCKET_ENDPOINTS)) {
      res.writeHead(403, { 'content-type': 'application/json', 'cache-control': 'no-store' });
      res.end('{"error":"pocket-rpc-local-only"}');
      return;
    }

    // 访问令牌认证（issue #13 + #18 + #33 + #40 + #69）：局域网与公网按开关/来源要求密码
    if (auth && !deviceChannel) {
    ...
    const proxyReq = httpRequest(
      ...
      (proxyRes) => {
        ...
        if (inject && contentType.includes('text/html') && !isCompressed(proxyRes.headers)) {
          const chunks = [];
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (deviceAuth && deviceAuth.handles(host)) {
      // 设备通道：WS 握手也必须带有效设备会话。漏了这一步，未认证的人就能
      // 直接开一条 WS 拿到实时事件流——等于认证形同虚设。
      if (!deviceAuth.authorizeUpgrade(req)) {
