...
import { createPocketProxy, DEFAULT_INJECT } from './proxy.mjs';
import { startQuickTunnel, startNamedTunnel } from './tunnel.mjs';
import { createRelayAgent, localAgentId } from './relay.mjs';
import { createDeviceAuth } from './device-auth.mjs';
import { createDeviceAuthGateway } from './device-auth-http.mjs';
...
/** 去掉 Host 的端口（[::1]:3081 → ::1；host:3081 → host），与 classifyHost 同款处理。 */
...
/** 从对外访问地址里取出主机名（解析失败返回空串 = 不匹配任何 Host）。 */
...
/** 对外访问地址本身（去掉尾部斜杠），用于二维码与设备配对链接。 */
function publicHostUrl(url) {
...
export function createPocketService({
  ...
  onTunnelReady,
  /** 中继配置：() => ({ enabled, consent, host, port, tls, allowInsecure, token, publicUrl }) */
  getRelayConfig,
  ...
  /** 中继 agent（自建 relay 通道）。配置好即常驻，与隧道生命周期完全独立。 */
  let relay = null;
  const createRelay = internals.createRelayAgent ?? createRelayAgent;

  // ---------- 设备认证（relay 通道专用） ----------
  // 网关常驻，但**是否生效按每个请求当时的 relay 配置判定**：用户改了对外地址
  // 或关掉中继，下一次请求就换行为，不需要重启代理。
  const readRelayCfg = () => (typeof getRelayConfig === 'function' ? (getRelayConfig() ?? {}) : {});
  const deviceAuthStore = internals.deviceAuth ?? createDeviceAuth();
  const deviceGateway = internals.deviceGateway ?? createDeviceAuthGateway({
    auth: deviceAuthStore,
    isDeviceHost: (host) => {
      const cfg = readRelayCfg();
      if (cfg.enabled !== true) return false;
      const want = publicHostOf(cfg.publicUrl);
      return want !== '' && hostNameOnly(host) === want;
    },
    publicUrl: () => readRelayCfg().publicUrl ?? '',
    log,
  });
  ...
  return {
      ...
      for (let p = port; p < port + 10; p++) {
        try {
          proxy = await createProxy({
            ...
            // 注入内容按 host 定制：设备通道要额外注入「真实用户活动」上报脚本
            // （会话续期只认真实操作，见 device-auth-http.mjs）。
            injectHtml: (host) => {
              const base = injectHtml === undefined ? DEFAULT_INJECT : injectHtml;
              return deviceGateway.handles(host) ? base + deviceGateway.activityScript() : base;
            },
            ...(auth ? { auth } : {}),
            // 设备认证通道（relay）：与共享 PIN 互斥，命中该 host 就整段跳过 PIN
          ...
          if (p !== port) {
            logInfo(`dsh-pocket-relay: port ${port} busy, proxy on ${p} | 端口 ${port} 被占用，代理改用 ${p}`);
          }
          break;
        } catch (err) {
     ...
     * 启动中继通道（幂等）。
     *
     * 「无人监听」的核心：配置好了就直接连，不等用户点任何按钮。
     * 与隧道不同，中继**没有** start/stop 的用户语义——它就是常驻的，
     * 用户唯一的操作是「暂停」（stopRelay）或改配置。
     */
    async startRelay() {
      const cfg = typeof getRelayConfig === 'function' ? (getRelayConfig() ?? {}) : {};
      if (!cfg.host || !cfg.port || !cfg.token) {
        throw new Error('中继未配置完整（地址 / 端口 / Token） | relay is not configured');
      }
      await this.startProxy(); // 代理是上游，必须先在
      if (relay) return relay.status();
      relay = createRelay({
        host: cfg.host,
        port: cfg.port,
        tls: cfg.tls === true,
        // 自签证书场景允许放行；代价是放弃中间人防护，界面上要讲清楚
        rejectUnauthorized: cfg.allowInsecure !== true,
        token: cfg.token,
        // 本机在 relay 上的名字：留空时按主机名派生。
        // 不能默认成 "default"：relay 按名字登记机器，同名会被后来者踢掉，
        // 多台电脑会互相踢到没法用（见 relay.mjs 的 localAgentId 注释）。
        agentId: cfg.agentId || localAgentId(),
        // 代理端口可能顺延（3081 被占 → 3082），所以实时取，不写死
        getTargetPort: () => proxy?.port ?? null,
        log,
      });
      relay.start();
      logInfo('dsh-pocket-relay: relay started → %s:%d | 中继已启动', cfg.host, cfg.port);
      return relay.status();
    },

    /** 停止中继（用户点「暂停」，或换配置时先停再起）。 */
    stopRelay() {
      const r = relay;
      relay = null;
      if (r) {
        try { r.stop(); } catch { /* 忽略 */ }
        logInfo('dsh-pocket-relay: relay stopped | 中继已停止');
      }
    },

    /** 中继状态快照（未启动时返回 idle）。 */
    relayStatus() {
      return relay ? relay.status() : { phase: 'idle', detail: '', streams: 0, reconnects: 0 };
    },

    /**
     * 设备认证存储（设置页的设备管理用：列出 / 生成配对码 / 批准 / 撤销）。
     *
     * 注意：它暴露的是**全部设备记录**（含哈希），只应被本机 loopback 的管理面使用。
     * 代理层已经保证非本机来源调不到 /dsh-pocket 通道（见 proxy.mjs 的
     * REMOTE_ALLOWED_POCKET_ENDPOINTS）。
     */
    deviceAuth: deviceAuthStore,

    /** 恢复出厂设置时一并撤销全部已配对设备（凭据属于本机状态，该一起清掉）。 */
    resetDevices() {
    ...
    async restoreTunnelIfNeeded() {
      logInfo('dsh-pocket-relay: auto-restore check | 自动恢复检查：读取上次隧道状态');
      ...
      if (!has) return;
      logInfo('dsh-pocket-relay: auto-restore: marker found, starting tunnel | 发现上次开启标记，尝试自动恢复');
      try {
        await this.startTunnel();
        logInfo('dsh-pocket-relay: public tunnel auto-restored | 已自动恢复公网隧道');
      } catch (err) {
        // 恢复失败保留标记（下次启动再试）；网络问题见 README 排障
        logWarn('dsh-pocket-relay: tunnel auto-restore failed | 自动恢复隧道失败: %s', err?.message ?? err);
      }
    },
    ...
    async status() {
      ...
      return {
        ...
        dshPort,
        // 中继通道状态（常开：配置好人就在线，界面上直接显示连没连上）
        relayRunning: relay !== null,
        relayState: this.relayStatus(),
        // 对外访问地址的二维码（地址固定，缓存一次长期复用）
        relayQr: await qrCached(publicHostUrl(readRelayCfg().publicUrl) || null),
      };
    },
